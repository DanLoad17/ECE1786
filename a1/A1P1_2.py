...
def download_glove(glove_dir='./glove', glove_dim=50):
    os.makedirs(glove_dir, exist_ok=True)  # create directory if it doesn't exist

    url = "http://nlp.stanford.edu/data/glove.6B.zip"
    zip_path = os.path.join(glove_dir, 'glove.6B.zip')

    if not os.path.exists(zip_path):  # download only if not already present
    ...
    if not os.path.exists(glove_path):  # extract only if needed
        print(f"Extracting {glove_file}...")
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extract(glove_file, glove_dir)

    return glove_path  # return path to requested glove file
...
def load_glove_vectors(glove_path, vocab_size=None):
    ...
    with open(glove_path, 'r', encoding='utf-8') as f:
        for i, line in enumerate(tqdm(f)):
            if vocab_size and i >= vocab_size:  # stop if vocab limit reached
                break
    ...
    embeddings = torch.from_numpy(np.array(vectors))  # convert to torch tensor
...
def glove(word):
    ...
    if idx is not None:
        return embeddings[idx]  # return vector for word
    else:
...
def print_closest_words(word, n=5):
    ...
    vec = embeddings[word2idx[word]]  # vector for input word
    dists = torch.norm(embeddings - vec, dim=1)  # Euclidean distances
    lst = sorted(enumerate(dists.numpy()), key=lambda x: x[1])  # sort by distance

    results = []
    for idx, difference in lst[1:n+1]:  # skip the word itself
...
def print_closest_cosine_words(word, N=5):
    ...
    word_vec = embeddings[word2idx[word]].unsqueeze(0)  # add batch dimension
    ...
    similarities = torch.mm(normed_embeddings, normed_word_vec.T).squeeze()  # cosine sim
    sorted_sim, sorted_idx = torch.sort(similarities, descending=True)

    results = []
    for i in range(1, N+1):  # skip the word itself at index 0
        sim_word = idx2word[sorted_idx[i].item()]
        results.append((sim_word, sorted_sim[i].item()))
    return results

if __name__ == "__main__":
    glove_path = download_glove(glove_dim=50)  # download and extract

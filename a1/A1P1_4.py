...
def download_glove(glove_dir='./glove', glove_dim=50):
    os.makedirs(glove_dir, exist_ok=True)  # ensure output directory exists

    url = "http://nlp.stanford.edu/data/glove.6B.zip"
    zip_path = os.path.join(glove_dir, 'glove.6B.zip')

    if not os.path.exists(zip_path):  # download only if needed
...
def load_glove_vectors(glove_path, vocab_size=None):
    ...
    word2idx = {}   # maps word -> index
    idx2word = []   # list index -> word
...
def glove(word):
    idx = word2idx.get(word)  # use global mapping as before
    if idx is not None:
...
def print_closest_words(vec, n=5):
    # compute Euclidean distance from vec to all embeddings
    dists = torch.norm(embeddings - vec, dim=1)
    lst = sorted(enumerate(dists.numpy()), key=lambda x: x[1])
    # skip the closest (self) and print next n
...
if __name__ == "__main__":
    ...
    for country in countries:
        print(f"\nCountry: {country}")
        # analogy vector: country - "country" + "profession"

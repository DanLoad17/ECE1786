...
def compare_word_to_category(word, category_words):
    # average embedding for a category and compare word to it
    word_vec = glove(word)
...
def softmax(x):
    # simple softmax for 1D numpy array
...
if __name__ == "__main__":
    ...
    for word in test_words:
        try:
            ...
            vec = np.array([colour_sim, temp_sim])  # raw similarity scores
            vec_prob = softmax(vec)  # normalize with softmax

...
class Trainer:
    ...
    def run(self):
        ...
        val_loader = DataLoader(
            ...
            sampler=torch.utils.data.RandomSampler(
                self.validation_dataset, replacement=True, num_samples=int(1e10)),
            shuffle=False,
            pin_memory=False,
            batch_size=config.batch_size,
            num_workers=config.num_workers,
        ...
        while True:
            ...
            batch_val = [t.to(self.device) for t in batch_val]
            x, y = batch
            ...
            model.eval()
            with torch.no_grad():
                logits, self.vloss = model(x_val, y_val, self.downstream_finetune)
                ypred = torch.argmax(F.softmax(logits, dim=-1), dim=1)

                ypred = ypred.view(-1)
                y = y.view(-1)
                correct = (ypred == y).sum().item()
                self.vacc = correct / y.size(0)
